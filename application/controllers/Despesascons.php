...
class Despesascons extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model(array('Basico_model', 'Servico_model', 'Produto_model', 'Produtos_model', 'ServicoBase_model', 'ProdutoBase_model', 'Produtocompra_model', 'Servicocompra_model', 'Despesascons_model', 'Tipodespesa_model', 'Profissional_model', 'Formapag_model', 'Cliente_model'));
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$data['select']['TipoDespesa'] = $this->Tipodespesa_model->select_tipodespesacons();
        ...
        $data['form_open_path'] = 'despesascons/cadastrar';
        $data['readonly'] = '';
        $data['disabled'] = '';
        $data['panel'] = 'primary';
        $data['metodo'] = 1;
        ...
        if ($this->form_validation->run() === FALSE) {
            //if (1 == 1) {
            $this->load->view('despesascons/form_despesascons', $data);
        } else {
            ...
            $data['despesas']['idApp_Consultor'] = $_SESSION['log']['id'];
            $data['despesas']['idTab_Modulo'] = $_SESSION['log']['idTab_Modulo'];
            $data['despesas']['idApp_Despesascons'] = $this->Despesascons_model->set_despesas($data['despesas']);
            /*
            ...
            */
            ...
            if (isset($data['servico'])) {
                ...
                for($j=1;$j<=$max;$j++) {
                    ...
                    $data['servico'][$j]['idApp_Despesascons'] = $data['despesas']['idApp_Despesascons'];
                ...
                $data['servico']['idApp_ServicoCompracons'] = $this->Despesascons_model->set_servico_compra($data['servico']);
            }

            #### App_ProdutoCompracons ####
            if (isset($data['produto'])) {
                ...
                for($j=1;$j<=$max;$j++) {
                    ...
                    $data['produto'][$j]['idApp_Despesascons'] = $data['despesas']['idApp_Despesascons'];
                ...
                $data['produto']['idApp_ProdutoCompracons'] = $this->Despesascons_model->set_produto_compra($data['produto']);
            }
            ...
            if (isset($data['parcelaspag'])) {
                ...
                for($j=1;$j<=$max;$j++) {
                    $data['parcelaspag'][$j]['idApp_Consultor'] = $_SESSION['log']['id'];
                    $data['parcelaspag'][$j]['Empresa'] = $_SESSION['log']['Empresa'];
					$data['parcelaspag'][$j]['idTab_Modulo'] = $_SESSION['log']['idTab_Modulo'];
                    $data['parcelaspag'][$j]['idApp_Despesascons'] = $data['despesas']['idApp_Despesascons'];
                ...
                $data['parcelaspag']['idApp_ParcelasPagaveiscons'] = $this->Despesascons_model->set_parcelaspag($data['parcelaspag']);
            }
...
*/
            ...
            if ($data['idApp_Despesascons'] === FALSE) {
                ...
                //$data['auditoriaitem'] = $this->basico->set_log($data['anterior'], $data['query'], $data['campos'], $data['idApp_Despesascons'], FALSE);
                //$data['auditoria'] = $this->Basico_model->set_auditoria($data['auditoriaitem'], 'App_Despesascons', 'CREATE', $data['auditoriaitem']);
                $data['msg'] = '?m=1';

                redirect(base_url() . 'relatorioconsultor/despesas/'  . $data['msg']);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        for ($i = 1; $i <= $data['count']['SCount']; $i++) {
            ...
            if ($this->input->post('idTab_Servico' . $i)) {
                $data['servico'][$j]['idApp_ServicoCompracons'] = $this->input->post('idApp_ServicoCompracons' . $i);
                $data['servico'][$j]['idTab_Servico'] = $this->input->post('idTab_Servico' . $i);
                $data['servico'][$j]['ValorCompraServico'] = $this->input->post('ValorCompraServico' . $i);
                $data['servico'][$j]['QtdCompraServico'] = $this->input->post('QtdCompraServico' . $i);
                $data['servico'][$j]['SubtotalServico'] = $this->input->post('SubtotalServico' . $i);
        ...
        for ($i = 1; $i <= $data['count']['PCount']; $i++) {
            ...
            if ($this->input->post('idTab_Produto' . $i) || $this->input->post('ValorCompraProduto' . $i) ||
					$this->input->post('QtdCompraProduto' . $i) || $this->input->post('SubtotalProduto' . $i) ||
					$this->input->post('ObsProduto' . $i) || $this->input->post('DataValidadeProduto' . $i)) {
                $data['produto'][$j]['idApp_ProdutoCompracons'] = $this->input->post('idApp_ProdutoCompracons' . $i);
                $data['produto'][$j]['idTab_Produto'] = $this->input->post('idTab_Produto' . $i);
                $data['produto'][$j]['ValorCompraProduto'] = $this->input->post('ValorCompraProduto' . $i);
                $data['produto'][$j]['QtdCompraProduto'] = $this->input->post('QtdCompraProduto' . $i);
                $data['produto'][$j]['SubtotalProduto'] = $this->input->post('SubtotalProduto' . $i);
        ...
        for ($i = 1; $i <= $data['count']['PRCount']; $i++) {
            ...
            if ($this->input->post('ParcelaPagaveis' . $i) || $this->input->post('ValorParcelaPagaveis' . $i) || 
                ...
                $data['parcelaspag'][$j]['idApp_ParcelasPagaveiscons'] = $this->input->post('idApp_ParcelasPagaveiscons' . $i);
        ...
        if ($id) {
            #### App_Despesascons ####
            $data['despesas'] = $this->Despesascons_model->get_despesas($id);
           ...
           # $_SESSION['Cliente'] = $this->Cliente_model->get_cliente($data['despesas']['idApp_Cliente'], TRUE);
            ...
            #### App_ServicoCompracons ####
            $data['servico'] = $this->Despesascons_model->get_servico($id);
            ...
            #### App_ProdutoCompracons ####
            $data['produto'] = $this->Despesascons_model->get_produto($id);
            ...
            #### App_ParcelasPagaveiscons ####
            $data['parcelaspag'] = $this->Despesascons_model->get_parcelaspag($id);
        ...
        $data['form_open_path'] = 'despesascons/alterar';
        $data['readonly'] = '';
        $data['disabled'] = '';
        $data['panel'] = 'primary';
        $data['metodo'] = 2;
        ...
        #run form validation
        if ($this->form_validation->run() === FALSE) {
            $this->load->view('despesascons/form_despesasalterarcons', $data);
        } else {
            ...
            $data['update']['despesas']['anterior'] = $this->Despesascons_model->get_despesas($data['despesas']['idApp_Despesascons']);
            ...
            $data['update']['despesas']['auditoriaitem'] = $this->basico->set_log(
                ...
                $data['despesas']['idApp_Despesascons'], TRUE);
            $data['update']['despesas']['bd'] = $this->Despesascons_model->update_despesas($data['despesas'], $data['despesas']['idApp_Despesascons']);

            #### App_ServicoCompracons ####
            $data['update']['servico']['anterior'] = $this->Despesascons_model->get_servico($data['despesas']['idApp_Despesascons']);
            if (isset($data['servico']) || (!isset($data['servico']) && isset($data['update']['servico']['anterior']) ) ) {
                ...
                $data['update']['servico'] = $this->basico->tratamento_array_multidimensional($data['servico'], $data['update']['servico']['anterior'], 'idApp_ServicoCompracons');
                ...
                for($j=0;$j<$max;$j++) {
                    ...
                    $data['update']['servico']['inserir'][$j]['idApp_Despesascons'] = $data['despesas']['idApp_Despesascons'];
                ...
                if (count($data['update']['servico']['inserir']))
                    $data['update']['servico']['bd']['inserir'] = $this->Despesascons_model->set_servico_compra($data['update']['servico']['inserir']);

                if (count($data['update']['servico']['alterar']))
                    $data['update']['servico']['bd']['alterar'] = $this->Despesascons_model->update_servico_compra($data['update']['servico']['alterar']);

                if (count($data['update']['servico']['excluir']))
                    $data['update']['servico']['bd']['excluir'] = $this->Despesascons_model->delete_servico_compra($data['update']['servico']['excluir']);
            }

            #### App_ProdutoCompracons ####
            $data['update']['produto']['anterior'] = $this->Despesascons_model->get_produto($data['despesas']['idApp_Despesascons']);
            if (isset($data['produto']) || (!isset($data['produto']) && isset($data['update']['produto']['anterior']) ) ) {
                ...
                //faz o tratamento da variável multidimensional, que ira separar o que deve ser inserido, alterado e excluído
                $data['update']['produto'] = $this->basico->tratamento_array_multidimensional($data['produto'], $data['update']['produto']['anterior'], 'idApp_ProdutoCompracons');
                ...
                for($j=0;$j<$max;$j++) {
                    ...
                    $data['update']['produto']['inserir'][$j]['idApp_Despesascons'] = $data['despesas']['idApp_Despesascons'];
                ...
                if (count($data['update']['produto']['inserir']))
                    $data['update']['produto']['bd']['inserir'] = $this->Despesascons_model->set_produto_compra($data['update']['produto']['inserir']);

                if (count($data['update']['produto']['alterar']))
                    $data['update']['produto']['bd']['alterar'] =  $this->Despesascons_model->update_produto_compra($data['update']['produto']['alterar']);

                if (count($data['update']['produto']['excluir']))
                    $data['update']['produto']['bd']['excluir'] = $this->Despesascons_model->delete_produto_compra($data['update']['produto']['excluir']);

            }

            #### App_ParcelasRec ####
            $data['update']['parcelaspag']['anterior'] = $this->Despesascons_model->get_parcelaspag($data['despesas']['idApp_Despesascons']);
            if (isset($data['parcelaspag']) || (!isset($data['parcelaspag']) && isset($data['update']['parcelaspag']['anterior']) ) ) {
                ...
                //faz o tratamento da variável multidimensional, que ira separar o que deve ser inserido, alterado e excluído
                $data['update']['parcelaspag'] = $this->basico->tratamento_array_multidimensional($data['parcelaspag'], $data['update']['parcelaspag']['anterior'], 'idApp_ParcelasPagaveiscons');

                $max = count($data['update']['parcelaspag']['inserir']);
                for($j=0;$j<$max;$j++) {
                    $data['update']['parcelaspag']['inserir'][$j]['idApp_Consultor'] = $_SESSION['log']['id'];
                    ...
                    $data['update']['parcelaspag']['inserir'][$j]['idApp_Despesascons'] = $data['despesas']['idApp_Despesascons'];
                ...
                if (count($data['update']['parcelaspag']['inserir']))
                    $data['update']['parcelaspag']['bd']['inserir'] = $this->Despesascons_model->set_parcelaspag($data['update']['parcelaspag']['inserir']);

                if (count($data['update']['parcelaspag']['alterar']))
                    $data['update']['parcelaspag']['bd']['alterar'] =  $this->Despesascons_model->update_parcelaspag($data['update']['parcelaspag']['alterar']);

                if (count($data['update']['parcelaspag']['excluir']))
                    $data['update']['parcelaspag']['bd']['excluir'] = $this->Despesascons_model->delete_parcelaspag($data['update']['parcelaspag']['excluir']);
...
*/
    ...
    public function excluir($id = FALSE) {
        ...
        elseif ($this->input->get('m') == 2)
            ...
            $data['msg'] = '';
                ...
                $this->Despesascons_model->delete_despesas($id);
    ...
    public function listar($id = NULL) {
        ...
        //$_SESSION['Despesas'] = $this->Despesascons_model->get_cliente($id, TRUE);
        //$_SESSION['Despesas']['idApp_Cliente'] = $id;
        $data['aprovado'] = $this->Despesascons_model->list_despesascons($id, 'S', TRUE);
        $data['naoaprovado'] = $this->Despesascons_model->list_despesascons($id, 'N', TRUE);
        ...
        $data['list'] = $this->load->view('despesas/list_despesascons', $data, TRUE);
    ...
    public function listarBKP($id = NULL) {
        ...
        //$_SESSION['Despesas'] = $this->Despesascons_model->get_cliente($id, TRUE);
        $_SESSION['Despesas']['idApp_Cliente'] = $id;
        $data['query'] = $this->Despesascons_model->list_despesascons(TRUE, TRUE);
        /*
          echo "<pre>";
          print_r($data['query']);
          echo "</pre>";
