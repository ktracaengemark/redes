...
class Relatorioconsultor_model extends CI_Model {
        ...
        $query = $this->db->query('
            ...
            FROM
                App_OrcaTrataCons AS OT
					LEFT JOIN App_Cliente AS C ON C.idApp_Cliente = OT.idApp_Cliente
            WHERE
				OT.idApp_Consultor = ' . $_SESSION['log']['id'] . ' 
            ORDER BY
                ' . $data['Campo'] . ' ' . $data['Ordenamento'] . '
...
*/
	...
	public function list_clientes($data, $completo) {
        ...
        $data['Campo'] = (!$data['Campo']) ? 'C.NomeCliente' : $data['Campo'];
        $data['Ordenamento'] = (!$data['Ordenamento']) ? 'ASC' : $data['Ordenamento'];
		#$filtro1 = ($data['Ativo'] != '#') ? 'C.Ativo = "' . $data['Ativo'] . '" AND ' : FALSE;
	...
	public function list_produtosconsultor($data, $completo) {
